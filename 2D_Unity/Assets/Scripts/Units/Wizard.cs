        ...
        if (0 < attackTimer[0])
        {
            ...
            if (attackTimer[0] <= 0.0f)
            {
                if (refTarget != null)
                {
                    ...
                    GameObject go = Instantiate(fireBullet);
                    go.tag = gameObject.tag;
                    //go.layer = gameObject.layer;

                    bool b = go.TryGetComponent(out Fire_Bullet outFire);

                    if (b == true)
                    {
                        outFire.SetTarget(gameObject, refTarget, Random.Range(damage[0], damage[1]));
                        refTarget = null;
                    }
